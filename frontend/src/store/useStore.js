...
export const useStore = create((set, get) => ({
  bars:      [],
  selectedBar: null,
  universes: {}, // { [universeNumber]: Uint8Array(512) }
  ...
  // Met à jour le buffer d'un univers DMX
  ...
  // Retourne les couleurs RGB d'une barre selon sa config
  getBarColors: (bar) => {
    const state   = get()
    ...
    if (mode === 2) {
      // Full Bar — 3 canaux pour toute la barre
    ...
    if (mode === 1) {
      ...
      for (let i = 0; i < ledCount; i++) {
        ...
        const r = u1data[ch]     || 0
        const g = u1data[ch + 1] || 0
        const b = u1data[ch + 2] || 0
        colors[i] = [r, g, b]
      }
      return colors
    }

    // Full Pixel — 3 canaux par LED
